...
void Fatfs_Test(void)
{
    ...
    FIL fp;
    uint32_t bw;
    char write_buff[32] = {"Aisinochip FreeRTOS FatFS Test!"};
    char read_buff[32];


    if (f_open(&fp, "test.txt", FA_WRITE | FA_READ | FA_OPEN_ALWAYS) != FR_OK) {
        printstrCom("open file fail\r\n");
        return ;
    }
    printstrCom("open file success\r\n");

    if (f_write(&fp, write_buff, 32, &bw) != FR_OK) {
        printstrCom("file write fail\r\n");
        return ;
    }
    printstrCom("file write success\r\n");

    if (f_lseek(&fp, 0) != FR_OK) { /* 移动读/写指针到文件首 */
        printstrCom("move ptr fail\r\n");
        return ;
    }
    if (f_read(&fp, read_buff, 32, &bw) != FR_OK) {
        printstrCom("file read fail\r\n");
        return ;
    }
    printstrCom("file read success\r\n");
    printf("read data:%s\r\n", read_buff);
    f_close(&fp);
}
