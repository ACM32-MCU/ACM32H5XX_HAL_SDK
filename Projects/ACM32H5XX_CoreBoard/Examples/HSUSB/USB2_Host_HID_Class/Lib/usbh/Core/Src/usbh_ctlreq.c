...
/**
  * @brief  USBH_Get_OtgDesc
  *         The OTG descriptor indicate whether the SRP, HNP and ADP feature are supported by B-device
  * @param  phost: Host Handle
  * @param  length: Length of the descriptor
  * @retval USBH Status
  */
USBH_StatusTypeDef USBH_Get_OtgDesc(USBH_HandleTypeDef *phost,
                                    uint16_t length)

{
#ifdef USB_OTG_FEATURE_EN
  USBH_StatusTypeDef status;
  uint8_t *pData = phost->device.Data;

  if ((status = USBH_GetDescriptor(phost, (USB_REQ_RECIPIENT_DEVICE | USB_REQ_TYPE_STANDARD),
                                   USB_DESC_OTG, pData, length)) == USBH_OK)
  {
  	phost->device.OtgAttr = pData[2];	//Attributes
  }
...
//	uart_printf("phost->Control.state=0x%x\n", phost->Control.state);
    ...
    case CTRL_ERROR:
      ...
      */
      ...
      else
      {
        ...
        uart_printf("Control error: Device not responding\n");
