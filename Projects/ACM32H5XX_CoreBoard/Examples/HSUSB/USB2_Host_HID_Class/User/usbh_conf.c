/**
  ...
  * @attention
  *
  * <h2><center>&copy; Copyright (c) 2017 AisinoChip International N.V.
...
void HAL_HCD_MspInit(HCD_HandleTypeDef *hhcd)
{
    ...
    if (USB_OTG_ULPI_PHY == hhcd->Init.phy_itface)
    {
    ...
    __HAL_RCC_GPIOC_CLK_ENABLE();     
	//use PC9 as power control pin
	GPIO_InitStruct.Pin 	  = USBH_PWR_CTRL_PIN;
    GPIO_InitStruct.Drive     = GPIO_DRIVE_LEVEL3;        
	GPIO_InitStruct.Mode	  = GPIO_MODE_OUTPUT_PP;
	GPIO_InitStruct.Pull	  = GPIO_PULLUP;
	GPIO_InitStruct.Alternate = GPIO_FUNCTION_0;
	HAL_GPIO_Init(USBH_PWR_CTRL_PORT, &GPIO_InitStruct);
    HAL_GPIO_WritePin(USBH_PWR_CTRL_PORT, USBH_PWR_CTRL_PIN, GPIO_PIN_SET);   
}
...
{
	if (USB_HS_OTG_ID1 == phost->id)
	{
		hhcd.Instance = USB_OTG_HS1;
	}
	else
	{
		hhcd.Instance = USB_OTG_HS2;
	}

    #ifdef USE_USB_SERIAL_PHY
...
//	hhcd.Init.low_power_enable = 0;
    ...
    #ifdef USE_USB_SERIAL_PHY
    hhcd.Init.speed = USBH_FSLS_SPEED;  
    #else
    hhcd.Init.speed = USBH_HS_SPEED;  
    #endif 
	hhcd.Init.vbus_sensing_enable = 0;
	hhcd.Init.use_external_vbus = 0;  
	hhcd.Init.lpm_enable = 0;
    hhcd.Init.phy_cfg = HCFG_30_MHZ;  
