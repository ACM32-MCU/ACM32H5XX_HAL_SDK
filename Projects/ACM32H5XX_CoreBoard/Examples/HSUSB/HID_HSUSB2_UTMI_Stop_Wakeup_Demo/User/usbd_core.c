...
USBD_StatusTypeDef USBD_LL_Bus_Activity(USBD_HandleTypeDef *pdev)
{
    ...
    if (p_OTG->GINTSTS & USB_OTG_GINTSTS_USBSUSP)
    {
        ...
        {
            USB_PrintfS("Suspend\n"); 
             USBOTG2_PHYC->CR = (BIT0 | BIT4); // PLL_EN Disable 
             *(uint32_t *)0x40080E00 = *(uint32_t *)0x40080E00 | BIT0;  // Stop PHY Clock 

            EXTI->IENR1 |= BIT24; 
            EXTI->FTENR1 |= BIT24;  
            EXTI->PDR1 = 0xFFFFFFFF;   
//            printfS("USBOTG2_PHYC:0x%x, 0x%x, 0x%x\n", USBOTG2_PHYC->CR, EXTI->IENR1, EXTI->FTENR1); 
