...
static USBH_StatusTypeDef USBH_HID_InterfaceInit(USBH_HandleTypeDef *phost)
{
  ...
  interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, HID_BOOT_CODE, 0xFFU, 0, HOST_INTF_EP0_VALID);
...
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost)
{
  ...
  switch (HID_Handle->state)
  {
    ...
    case HID_POLL:
      if ((Ret=USBH_LL_GetURBState(phost, HID_Handle->InPipe)) == USBH_URB_DONE)
      {
