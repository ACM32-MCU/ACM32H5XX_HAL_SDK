...
#define BYTES_TO_WORDS_4(a, b, c, d) 0x##d##c##b##a##ull
    ...
    uECC_SUPPORTS_secp224r1 || uECC_SUPPORTS_secp256r1  || uECC_SUPPORTS_sm2
...
#if uECC_SUPPORTS_secp160r1 || uECC_SUPPORTS_secp192r1 || \
    uECC_SUPPORTS_secp256r1 || uECC_SUPPORTS_secp256k1 || uECC_SUPPORTS_sm2
...
#if (uECC_OPTIMIZATION_LEVEL > 0)
static void vli_mmod_fast_sm2(uECC_word_t *result, uECC_word_t *product);
#endif

static const struct uECC_Curve_t curve_sm2 = {
    num_words_sm2,
    num_bytes_sm2,
    256, /* num_n_bits */
    {   BYTES_TO_WORDS_8(FF, FF, FF, FF, FF, FF, FF, FF),
        BYTES_TO_WORDS_8(00, 00, 00, 00, FF, FF, FF, FF),
        BYTES_TO_WORDS_8(FF, FF, FF, FF, FF, FF, FF, FF),
        BYTES_TO_WORDS_8(FF, FF, FF, FF, FE, FF, FF, FF) },
    {   BYTES_TO_WORDS_8(23, 41, D5, 39, 09, F4, BB, 53),
        BYTES_TO_WORDS_8(2B, 05, C6, 21, 6B, DF, 03, 72),
        BYTES_TO_WORDS_8(FF, FF, FF, FF, FF, FF, FF, FF),
        BYTES_TO_WORDS_8(FF, FF, FF, FF, FE, FF, FF, FF) },

    {   BYTES_TO_WORDS_8(C7, 74, 4C, 33, 89, 45, 5A, 71),
        BYTES_TO_WORDS_8(E1, 0B, 66, F2, BF, 0B, E3, 8F),
        BYTES_TO_WORDS_8(94, C9, 39, 6A, 46, 04, 99, 5F),
        BYTES_TO_WORDS_8(19, 81, 19, 1F, 2C, AE, C4, 32),

        BYTES_TO_WORDS_8(A0, F0, 39, 21, E5, 32, DF, 02),
        BYTES_TO_WORDS_8(40, 47, 2A, C6 ,7C ,87 ,A9 ,D0),
        BYTES_TO_WORDS_8(53 ,21 ,69 ,6B ,E3 ,CE ,BD ,59),
        BYTES_TO_WORDS_8(9C ,77 ,F6 ,F4 ,A2 ,36 ,37 ,BC) },
    {   BYTES_TO_WORDS_8(93, 0E, 94, 4D, 41, BD, BC, DD),
        BYTES_TO_WORDS_8(92, 8F, AB, 15, F5, 89, 97, F3),
        BYTES_TO_WORDS_8(A7, 09, 65, CF, 4B, 9E, 5A, 4D),
        BYTES_TO_WORDS_8(34, 5E, 9F, 9D, 9E, FA, E9, 28) },
    &double_jacobian_default,
#if uECC_SUPPORT_COMPRESSED_POINT
    &mod_sqrt_default,
#endif
...
uECC_Curve uECC_sm2(void) { return &curve_sm2; }


#if (uECC_OPTIMIZATION_LEVEL > 0 && !asm_mmod_fast_sm2)
/* Computes result = product % curve_p
   from http://www.nsa.gov/ia/_files/nist-routines.pdf */
#if uECC_WORD_SIZE == 1
static void vli_mmod_fast_sm2(uint8_t *result, uint8_t *product) {
    uint8_t tmp[num_words_sm2];
    int8_t carry;
    
    /* t */
    uECC_vli_set(result, product, num_words_sm2);
    ...
    tmp[24] = product[56]; tmp[25] = product[57]; tmp[26] = product[58]; tmp[27] = product[59];
    tmp[28] = product[60]; tmp[29] = product[61]; tmp[30] = product[62]; tmp[31] = product[63];
    carry = uECC_vli_add(tmp, tmp, tmp, num_words_sm2);
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);
    ...
    tmp[24] = product[32]; tmp[25] = product[33]; tmp[26] = product[34]; tmp[27] = product[35];
    tmp[28] = product[40]; tmp[29] = product[41]; tmp[30] = product[42]; tmp[31] = product[43];
    carry -= uECC_vli_sub(result, result, tmp, num_words_sm2);
    ...
    if (carry < 0) {
        do {
            carry += uECC_vli_add(result, result, curve_sm2.p, num_words_sm2);
        } while (carry < 0);
    } else {
        while (carry || uECC_vli_cmp_unsafe(curve_sm2.p, result, num_words_sm2) != 1) {
            carry -= uECC_vli_sub(result, result, curve_sm2.p, num_words_sm2);
        }
    }
}
#elif uECC_WORD_SIZE == 4
static void vli_mmod_fast_sm2(uint32_t *result, uint32_t *product) {
    uint32_t tmp[num_words_sm2];
    int carry;
 ...
 /* s2 */
    ...
    tmp[6] = product[14];
    tmp[7] = product[15];
    carry = uECC_vli_add(result, result, tmp, num_words_sm2); //s1+s2
    ...
    tmp[7] = product[14];
    carry += uECC_vli_add(tmp, tmp, tmp, num_words_sm2);
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3
    ...
    tmp[6] = 0;
    tmp[7] = product[13];
    carry += uECC_vli_add(tmp, tmp, tmp, num_words_sm2); //2*s4
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4
    ...
    tmp[6] = 0;
    tmp[7] = product[12];
    carry += uECC_vli_add(tmp, tmp, tmp, num_words_sm2); //2*s5
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5
    ...
    tmp[6] = product[11];
    tmp[7] = product[11];
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s*6
    ...
    tmp[6] = product[15];
    tmp[7] = product[10];
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s*6+s*7
    ...
    tmp[6] = 0;
    tmp[7] = product[9];
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s*6+s*7+s*8
    ...
    tmp[6] = 0;
    tmp[7] = product[15];
    carry += uECC_vli_add(tmp, tmp, tmp, num_words_sm2); //2*s10
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s*6+s*7+s*8+2*s10
    ...
    tmp[6] = 0;
    tmp[7] = 0;
    carry -= uECC_vli_sub(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s*6+s*7+s*8+2*s10-s11
    //s12
    tmp[0] = 0;
    tmp[1] = 0;
    tmp[2] = product[13];
    ...
    tmp[6] = 0;
    tmp[7] = 0;
    carry -= uECC_vli_sub(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s*6+s*7+s*8+2*s10-s11-s12
    //s13
    tmp[0] = 0;
    tmp[1] = 0;
    tmp[2] = product[9];
    ...
    tmp[6] = 0;
    tmp[7] = 0;
    carry -= uECC_vli_sub(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s*6+s*7+s*8+2*s10-s11-s12-s13
    ...
    tmp[6] = 0;
    tmp[7] = 0;
    carry -= uECC_vli_sub(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s*6+s*7+s*8+2*s10-s11-s12-s13-s14
...
#else
static void vli_mmod_fast_sm2(uint64_t *result, uint64_t *product) {
    uint64_t tmp[num_words_sm2];
    int carry;
    ...
    /* s3 */
    tmp[0] = (product[6] >> 32) | (product[7] << 32);
    tmp[1] = (product[6] ) & 0xffffffff00000000ull;
    tmp[2] = product[7];
    tmp[3] = (product[7] << 32) & 0xffffffff00000000ull;
    carry += uECC_vli_add(tmp, tmp, tmp, num_words_sm2);
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3
    ...
    tmp[0] = product[6];
    tmp[1] = product[7] << 32;
    tmp[2] = (product[5] << 32) | (product[7] >> 32);
    tmp[3] = (product[5] & 0xffffffff00000000ull) | (product[5] >> 32);
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s6

    /* s7 */
    tmp[0] = product[5];
    tmp[1] = product[6] << 32;
    tmp[2] = (product[6] >> 32) | (product[7] << 32);
    tmp[3] = (product[5] << 32) | (product[7] >> 32);
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s6+s7

    /* s8 */
    tmp[0] = (product[4] >> 32) | (product[5] << 32);
    tmp[1] = (product[4] << 32);
    tmp[2] = (product[4] >> 32);
    tmp[3] = (product[4]) & 0xffffffff00000000ull;
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s6+s7+s8

    /* s9 */
    tmp[0] = (product[5] >> 32) | (product[6] << 32);
    tmp[1] = (product[7]) & 0xffffffff00000000ull;
    tmp[2] = 0;
    tmp[3] = (product[4] << 32);
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s6+s7+s8+s9

    /* s10 */
    tmp[0] = 0;
    tmp[1] = 0;
    tmp[2] = 0;
    tmp[3] = (product[7]) & 0xffffffff00000000ull;
    carry += uECC_vli_add(tmp, tmp, tmp, num_words_sm2); //2*s10
    carry += uECC_vli_add(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s6+s7+s8+s9+2*s10

    /* s11 */
    tmp[0] = 0;
    tmp[1] = product[7] & 0xffffffff;
    tmp[2] = 0;
    tmp[3] = 0;
    carry -= uECC_vli_sub(result, result, tmp, num_words_sm2);//s1+s2+2*s3+2*s4+2*s5+s6+s7+s8+s9+2*s10-s11
...
#endif /* uECC_WORD_SIZE */
#endif /* (uECC_OPTIMIZATION_LEVEL > 0 && !asm_mmod_fast_sm2) */
