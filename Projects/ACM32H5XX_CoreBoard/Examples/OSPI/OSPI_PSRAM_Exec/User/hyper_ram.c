...
void Hyper_OSPI_Init(void)
{
    ...
    /* 分频系数 */   
    HYPER_Handle.Init.BaudRatePrescaler  = OSPI_BAUDRATE_PRESCALER_2;
    /* 采样延时 */     
    HYPER_Handle.Init.SampleShifting     = OSPI_SAMPLE_SHIFT_2_5HCLK;
    /* FIFO写模式 */     
...
{   
    ...
    /* 数据选通使能 */                  
    HYPER_OSPI_Octal.DQSMode = OSPI_DQSOE_ENABLE;
    /* 存储器类型 */               
...
void W958D8NBYA_WriteRegister(uint8_t *pCA, uint16_t pData)
{
    ...
    HAL_OSPI_DQSOutputEnableDisable(&HYPER_Handle, OSPI_DQSOE_DISABLE);//Hyperbus存储器RWDS输出禁止
    HAL_OSPI_TransmitByHalfWord(&HYPER_Handle, write_buff, 4, 0);
    HAL_OSPI_DQSOutputEnableDisable(&HYPER_Handle, OSPI_DQSOE_ENABLE);//Hyperbus存储器RWDS输出使能  
}
...
void W958D8NBYA_Fifo_Read_Write_Test(void)
{
    ...
    for(i = 0; i < 16; i++)
    {
        printfS("Read[%d] = 0x%04x\r\n", i, readBuf[i]);
        if(readBuf[i] != writeBuf[i])
        {
            printfS("[%d]: readBuf[0x%04x] != writeBuf[0x%04x]\r\n", (i), readBuf[i], writeBuf[i]);
            err_count++;
        }
    }
    ...
    if(err_count)
    {
        printfS("W958D8NBYA FIFO Read Write Fail!!!\r\n");
    }
    else
    {
        printfS("W958D8NBYA FIFO Read Write Success!!!\r\n");
    }
    printfS("************************************************\r\n"); 
}
...
void W958D8NBYA_OSPI_Memory_Init(void)
{
    ...
    /* 突发长度（Burst Length）*/
    Hyper_Memeory_Handle.BurstLen = MEMOACC2_BURST_LEN_32;                    
    /* Hyperbus或xSPI模式下，RWDS为一时的LC周期数 */
    Hyper_Memeory_Handle.HyperXspiLC1 = MEMOACC2_LATENCY1_13;                
    /* Hyperbus或xSPI模式下，RWDS为零时的LC周期数 */ 
    Hyper_Memeory_Handle.HyperXspiLc0 = MEMOACC2_LATENCY0_6;                     
    
    /* [27]突发是Linear的还是Wrapped的 */
	Hyper_Memeory_Handle.HyperBurstType = 0;               
	/* [26:25]数据模式，八线模式时，此位无效 */  
    Hyper_Memeory_Handle.DataMode = 0;                    
	/* [24:23]交替字节模式，八线模式时，此位无效 */
    Hyper_Memeory_Handle.AlterByteMode = 0;                
	 /* [22:21]地址模式，八线模式时，此位无效 */  
    Hyper_Memeory_Handle.AddrMode = 0;                    
	/* [20:19]指令模式，八线模式时，此位无效 */ 
    Hyper_Memeory_Handle.InstrMode = 0;                     
	/* [18:17]地址长度（Hyper必须为32 bit） */
    Hyper_Memeory_Handle.AddrWidth = MEMOACC1_ADDR_WIDTH_32;                    
	/* [16:12]dummy周期长度（Hyper/xSPI无效） */
    Hyper_Memeory_Handle.DummyCycleSize = 0;               
	/* [11]读操作空指令字节使能（Hyper必须为1）,dummy使能位,有dummy时必须置1 */ 
    Hyper_Memeory_Handle.ReadDummyByteEnable = MEMOACC1_READ_DUMMY_ENABLE;                  
	/* [10]写操作空指令字节使能（Hyper必须为1）,dummy使能位,有dummy时必须置1 */  
    Hyper_Memeory_Handle.WriteummyByteEnable = MEMOACC1_WRITE_DUMMY_ENABLE;                       
	/* [8:7]]交替字节长度 8 16 24 32，八线模式时，此位无效 */
    Hyper_Memeory_Handle.AlterByteSize = 0;               		
	/* [6]读操作交替字节使能位，八线模式时，此位无效 */  
    Hyper_Memeory_Handle.ReadAlterByteEnable = 0;                       
	/* [5]写操作交替字节使能位，八线模式时，此位无效 */
    Hyper_Memeory_Handle.WriteAlterByteEnable = 0;                       
	/* [4]仅发送指令一次 */  
    Hyper_Memeory_Handle.SendInstrOnce = 0;                   
    /* [3]连续读使能 */
    Hyper_Memeory_Handle.ContinuousModeEnable = MEMOACC1_CON_ENABLE;                   
	/* [2]连续模式下CS拉高等待功能不使能 */
    Hyper_Memeory_Handle.CsTimeoutEnable = 0;
    //[2]连续模式下CS拉高等待功能不使能
	HAL_OSPI_MemoryInit(&HYPER_Handle, &Hyper_Memeory_Handle);                            
}

/******************************************************************************
* @brief : W958D8NBYA DTR Memory Mode 8-bit Read Write Test.
* @param : None
* @return: None
******************************************************************************/
void W958D8NBYA_Memory_8bit_Write_Read(void)
{
    ...
    if(err_count)
    {
        printfS("W958D8NBYA Memory 16-bit write & read fail!!!\r\n");
    }
    else
    {
        printfS("W958D8NBYA Memory 16-bit write & read Success!!!\r\n");
    }
    printfS("************************************************\r\n"); 
}

/******************************************************************************
* @brief : W958D8NBYA DTR Memory Mode 16-bit Read Write Test.
* @param : None
* @return: None
******************************************************************************/
...
/******************************************************************************
* @brief : W958D8NBYA DTR Memory Mode 32-bit Read Write Test.
* @param : None
* @return: None
******************************************************************************/
void W958D8NBYA_Memory_32bit_Write_Read(void)
{
    ...
    for(i = 0; i < HYPER_BUFF_LEN; i++)
    {
        if(readBuf[i] != writeBuf[i])
        {
            printfS("[%d]: readBuf[0x%08x] != writeBuf[0x%08x]\r\n", (i), readBuf[i], writeBuf[i]);
            err_count++;
        }
    }
    ...
    if(err_count)
    {
        printfS("W958D8NBYA Memory 32-bit write & read fail!!!\r\n");
    }
    else
    {
        printfS("W958D8NBYA Memory 32-bit write & read Success!!!\r\n");
    }
    printfS("************************************************\r\n"); 
}
