...
void APS6408L_OBM_OSPI_Init(void)
{
    ...
    /* 采样延时 */     
    XCCELAOPI_Handle.Init.SampleShifting     = OSPI_SAMPLE_SHIFT_2_5HCLK;
    /* FIFO写模式 */     
...
* @return: None
******************************************************************************/
void APS6408L_OBM_OSPI_DTR_Memory_16bit_Write_Read(void)
{    
    volatile uint16_t *pMem16;
    uint16_t i, err_count = 0;
    ...
    pMem16 = (volatile uint16_t *) (OSPI_MEMORY_ADDR + 0x2000);
    for(i = 0; i < OBM_BUFF_LEN; i++)
    {
        writeBuf[i] = (OBM_DATA_16BIT + i);
    }  
    ...
    if(err_count)
    {
        printfS("APS6408L_OBM Memory 16-bit read fail!!!\r\n");
        test_fail_flag = 1;
    }
    else
    {
        printfS("APS6408L_OBM Memory 16-bit read Success!!!\r\n");
    }
    printfS("************************************************\r\n");   
...
* @return: None
******************************************************************************/
void APS6408L_OBM_OSPI_DTR_Memory_32bit_Write_Read(void)
{
    ...
    pMem32 = (volatile uint32_t *) (OSPI_MEMORY_ADDR + 0x3000);

    for(i = 0; i < OBM_BUFF_LEN; i++)
    {
        writeBuf[i] = (OBM_DATA_32BIT + i);
    }
    ...
    if(err_count)
    {
        printfS("APS6408L_OBM Memory 32-bit read fail!!!\r\n");
        test_fail_flag = 1;
    }
    else
    {
        printfS("APS6408L_OBM Memory 32-bit read Success!!!\r\n");
    }
    printfS("************************************************\r\n");  
...
uint8_t sram8Buff[OBM_BUFF_LEN*64];
uint16_t sram16Buff[OBM_BUFF_LEN*64];
uint32_t sram32Buff[OBM_BUFF_LEN*64];
...
void APS6408L_OBM_OSPI_DTR_Memory_8bit_Write_Read(void)
{
    ...
    pMem8 = ( uint8_t *) (OSPI_MEMORY_ADDR + 0x1000);
    printfS("psram 8bit continue test\r\n");
    ram_8bit_write_read_test(pMem8,0);
    printfS("psram 8bit jump test\r\n");    
    ram_8bit_write_read_test(pMem8,1); 
    printfS("isram 8bit continue test\r\n");      
    ram_8bit_write_read_test(sram8Buff,0);
    printfS("isram 8bit jump test\r\n");      
    ram_8bit_write_read_test(sram8Buff,1);    
...
void APS6408L_OBM_OSPI_DTR_Memory_16bit_Write_Read(void)
{    
    volatile uint16_t *pMem;
    
    HAL_OSPI_MemoryEnableDisable(&XCCELAOPI_Handle,  MEMOACC1_MEMORY_ENABLE); //[1]存储器访问使能
    
    pMem = ( uint16_t *) (OSPI_MEMORY_ADDR + 0x1000);
    printfS("psram 16bit continue test\r\n");
    ram_16bit_write_read_test(pMem,0);
    printfS("psram 16bit jump test\r\n");    
    ram_16bit_write_read_test(pMem,1); 
    printfS("isram 16bit continue test\r\n");      
    ram_16bit_write_read_test(sram16Buff,0); 
    printfS("isram 16bit jump test\r\n");     
    ram_16bit_write_read_test(sram16Buff,1);     
}
...
void APS6408L_OBM_OSPI_DTR_Memory_32bit_Write_Read(void)
{
    ...
    pMem = ( uint32_t *) (OSPI_MEMORY_ADDR + 0x1000);
    printfS("psram 32bit continue test\r\n");
    ram_32bit_write_read_test(pMem,0);
    printfS("psram 32bit jump test\r\n");    
    ram_32bit_write_read_test(pMem,1); 
    printfS("isram 32bit continue test\r\n");      
    ram_32bit_write_read_test(sram32Buff,0);
    printfS("isram 32bit jump test\r\n");       
    ram_32bit_write_read_test(sram32Buff,1);    
}
...
{
    volatile uint8_t *p8Mem;
    volatile uint16_t *p16Mem;
    volatile uint32_t *p32Mem;    
    ...
    p8Mem = ( uint8_t *) (OSPI_MEMORY_ADDR + 0x1000);
    p16Mem = ( uint16_t *) (OSPI_MEMORY_ADDR + 0x1000);
    p32Mem = ( uint32_t *) (OSPI_MEMORY_ADDR + 0x1000);    
    
    
    printfS("*************************************\r\n");
    printfS("psram 8bit continue test\r\n");
    ram_8bit_write_read_test(p8Mem,0);
    printfS("psram 8bit jump test\r\n");    
    ram_8bit_write_read_test(p8Mem,1); 
    printfS("isram 8bit continue test\r\n");      
    ram_8bit_write_read_test(sram8Buff,0);
    printfS("isram 8bit jump test\r\n");      
    ram_8bit_write_read_test(sram8Buff,1); 
    ...
    printfS("*************************************\r\n");
    printfS("psram 16bit continue test\r\n");
    ram_16bit_write_read_test(p16Mem,0);
    printfS("psram 16bit jump test\r\n");    
    ram_16bit_write_read_test(p16Mem,1); 
    printfS("isram 16bit continue test\r\n");      
    ram_16bit_write_read_test(sram16Buff,0); 
    printfS("isram 16bit jump test\r\n");     
    ram_16bit_write_read_test(sram16Buff,1); 
    ...
    printfS("*************************************\r\n");
    printfS("psram 32bit continue test\r\n");
    ram_32bit_write_read_test(p32Mem,0);
    printfS("psram 32bit jump test\r\n");    
    ram_32bit_write_read_test(p32Mem,1); 
    printfS("isram 32bit continue test\r\n");      
    ram_32bit_write_read_test(sram32Buff,0);
    printfS("isram 32bit jump test\r\n");       
    ram_32bit_write_read_test(sram32Buff,1); 
