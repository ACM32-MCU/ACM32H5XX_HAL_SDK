...
#define SDRAM_BANK_TEST_ADDR            (SDRAM_BANK_ADDR + 0x100000U)     
...
#ifdef LQFP216    
    /* 读延迟配置 */
    hsdram_Init.ReadPipeDelay = FMC_SDRAM_RPIPE_DELAY_2;
#else
...
#ifdef LQFP216 
    FMC_SDRAM->SDRSMA2 = 0xffffffff;//提前0.5HCLK采样
#endif
    //Read_Values_From_Registers();
...
void SDRAM_32168Write_Read_Test(void)
{
    ...
    {
        *(__IO uint8_t*) (SDRAM_BANK_TEST_ADDR + counter) = (uint8_t)(0x0);
    }
    /* 向整个SDRAM写入数据 8位 */
    for (counter = 0; counter < SDRAM_SIZE; counter++)
    {
        *(__IO uint8_t*) (SDRAM_BANK_TEST_ADDR + counter) = (uint8_t)(u8WriteData + counter);
    }
    
    /* 读取 SDRAM 数据并检测*/
    for(counter = 0; counter < SDRAM_SIZE; counter++ )
    {
        u8ReadData = *(__IO uint8_t*)(SDRAM_BANK_TEST_ADDR + counter);  //从该地址读出数据
    ...
    {
        *(__IO uint16_t*) (SDRAM_BANK_TEST_ADDR + counter) = (uint16_t)(0);
    }    
    /* 向整个SDRAM写入数据 16位 */
    for (counter = 0; counter < SDRAM_SIZE; counter += 2)
    {
        *(__IO uint16_t*) (SDRAM_BANK_TEST_ADDR + counter) = (uint16_t)(u16WriteData + counter/2);
    }
    /* 读取 SDRAM 数据并检测*/
    for(counter = 0; counter < SDRAM_SIZE; counter += 2)
    {
        u16ReadData = *(__IO uint16_t*)(SDRAM_BANK_TEST_ADDR + counter);  //从该地址读出数据
    ...
    {
        *(__IO uint32_t*) (SDRAM_BANK_TEST_ADDR + counter) = (uint32_t)(0);
    }
    /* 向整个SDRAM写入数据 32位 */
    for (counter = 0; counter < SDRAM_SIZE; counter += 4)
    {
        *(__IO uint32_t*) (SDRAM_BANK_TEST_ADDR + counter) = (uint32_t)(u32WriteData + counter/4);
    }
    /* 读取 SDRAM 数据并检测*/
    for(counter = 0; counter < SDRAM_SIZE; counter += 4)
    {
        u32ReadData = *(__IO uint32_t*)(SDRAM_BANK_TEST_ADDR + counter);  //从该地址读出数据
...
{  
    ...
    {
        *(__IO uint32_t*) (SDRAM_BANK_TEST_ADDR + i) = (uint32_t)(0x00000000);
    }
    HAL_SimpleDelay(5000);
    
    HAL_DMA_Start(&DMA1_Ch0Handle, (uint32_t)writeBuf, SDRAM_BANK_TEST_ADDR, 24);
    ...
    for(i = 0; i< 24*4; i+=4 )
    {
        readdata = *(__IO uint32_t*)(SDRAM_BANK_TEST_ADDR + i);  //从该地址读出数据
    ...
    HAL_SimpleDelay(5000);
    HAL_DMA_Start(&DMA2_Ch1Handle, SDRAM_BANK_TEST_ADDR, (uint32_t)readBuf, 24);
    /* Wait Transfer Finish */
