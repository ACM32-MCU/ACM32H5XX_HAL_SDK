...
//  p[0] = 0x3;	     /* Enable systick, interrupt */
    NVIC_ClearPendingIRQ(SysTick_IRQn);
    NVIC_SetPriority(SysTick_IRQn, 0);
    NVIC_EnableIRQ(SysTick_IRQn);  
    SysTick_Config(0x00ffffff);
    Cormark_Begin_Time = SysTick->VAL;
#endif
#ifdef EXPECTED_DWT
...
#endif
//	stop_time();
//	total_ticks=get_time();

//	Timer_Stop();
//	Timer_Close();
#ifdef EXPECTED_SYST
    Cormark_End_Time = SysTick->VAL;  // Get current value;
    SysTick->CTRL=0;
    total_ticks= 0x1000000*systick_cyc +Cormark_Begin_Time - Cormark_End_Time;
#endif

#ifdef EXPECTED_DWT
    Cormark_End_Time = DWT->CYCCNT;  // Get current value;
    DWT->CYCCNT = 0;
    total_ticks= Cormark_End_Time - Cormark_Begin_Time;
#endif
...
#if HAS_FLOAT
	printf("Total time (secs): %f\r\n",time_in_secs(total_ticks));
	if (time_in_secs(total_ticks) > 0)
		printf("Iterations/Sec   : %f\r\n",default_num_contexts*results[0].iterations/time_in_secs(total_ticks));
#else
	printf("Total time (secs): %d\r\n",time_in_secs(total_ticks));
	if (time_in_secs(total_ticks) > 0)
		printf("Iterations/Sec   : %d\r\n",default_num_contexts*results[0].iterations/time_in_secs(total_ticks));
#endif
	if (time_in_secs(total_ticks) < 10) {
...
#if HAS_FLOAT
		if (known_id==3) {
			printf("CoreMark 1.0 : %f / %s %s",default_num_contexts*results[0].iterations/time_in_secs(total_ticks),COMPILER_VERSION,COMPILER_FLAGS);
...
#endif
			printf("\r\n");
//            extern int drv_get_sys_freq(void);
            printf("Score (Coremarks/%dMHz): %6.2f\r\n", (CoreMark_CLOCKS_PER_SEC/ 1000000),default_num_contexts*results[0].iterations/time_in_secs(total_ticks) / (CoreMark_CLOCKS_PER_SEC/ 1000000));
		}
#endif
	}
	if (total_errors>0)
