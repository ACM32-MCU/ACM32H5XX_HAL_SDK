...
uint32_t OSPI_MEMORY_ADDR = 0;
GPIO_TypeDef * xccela_rstn_gpio;
...
void APS6408L_OBM_OSPI_Init(void)
{
    ...
    /* For ACM32H5 EVB */
    if(IC_H528IIT7_G == chip_id_code)
    {
        __HAL_RCC_OSPI1_CLK_ENABLE();
        __HAL_RCC_OSPI1_RESET();  
        OSPI_MEMORY_ADDR = OSPI1_MEM_BASE_ADDR;           
        XCCELAOPI_Handle.Instance            = OSPI1;                
    }
    else /* For ACM32H5 CoreBoard */
    {
        __HAL_RCC_OSPI2_CLK_ENABLE();
        __HAL_RCC_OSPI2_RESET(); 
        OSPI_MEMORY_ADDR = OSPI2_MEM_BASE_ADDR;        
    ...
    /* 采样延时 */     
    XCCELAOPI_Handle.Init.SampleShifting     = OSPI_SAMPLE_SHIFT_3HCLK;  //PSRAM is SOC, use OSPI_SAMPLE_SHIFT_2_5HCLK    
    /* FIFO写模式 */     
    ...
    if(IC_H528IIT7_G == chip_id_code)
    {
        XCCELAOPI_Handle.Reset_Pin = GPIO_PIN_12;  
        xccela_rstn_gpio = GPIOC;        
    }
    else /* For ACM32H5 CoreBoard */
    {
        XCCELAOPI_Handle.Reset_Pin = GPIO_PIN_0;
        xccela_rstn_gpio = GPION;
    }
...
//    write_buff[pos++] = 0x0000;//dummy cycle 5     
...
/******************************************************************************
* @brief : APS6408L-OBM-BA fifo read and write test.
* @param : None
* @return: None
******************************************************************************/
void APS6408L_OBM_Fifo_Read_Write(void)
{
    uint16_t i, err_count = 0;
    uint16_t writeBuf[128]; 
    uint16_t readBuf[16] = { 0 };                            
    uint32_t test_addr = 0x000002;
    ...
    APS6408L_OBM_Synchronous_Write(test_addr, writeBuf, 128); 
    
    APS6408L_OBM_Synchronous_Read(test_addr, readBuf, 8);
    ...
    if(err_count)
    {
        printfS("APS6408L_OBM FIFO Read Write Fail!!!\r\n");
    }
    else
    {
        printfS("APS6408L_OBM FIFO Read Write Success!!!\r\n");
    }
    printfS("************************************************\r\n");
}
...
/******************************************************************************
* @brief : APS6408L-OBM-BA DTR Memory Mode 8-bit Read Write Test. 
           Write and Read 1MB as 8-bit Mode
* @param : write_data: the data write to PSRAM 
* @return: 0: PASS; 1: FAIL  
******************************************************************************/
uint16_t OSPI_DTR_Memory_8bit_Write_Read_Performance_Test(uint8_t write_data)
{    
    volatile uint8_t *pMem8;
    uint32_t i, ms_tick1, ms_tick2, ms_tick3, ms_tick4;  
 
    printfS("\r\n8bit_Write_Read_Performance_Test, 1M Bytes, Test Data: 0x%02x\r\n", write_data);    
    ...
    ////Start Read  
    ms_tick3 = HAL_GetTick();  
    for(i = 0; i < 0x100000; i++)   
    { 
        if (write_data != *(pMem8 + i) )   
        {
            printfS("Read Data Error: %d, 0x%x\r\n", i, *(pMem8 + i) );       
            return 1;  
        } 
    }       
    ms_tick4 = HAL_GetTick();     
    
    printfS("tick: %d, %d, %d, %d\n", ms_tick1, ms_tick2, ms_tick3, ms_tick4);   
    printfS("Write Time:%dms, Read Time:%dms\n", ms_tick2-ms_tick1, ms_tick4-ms_tick3);    
...
/******************************************************************************
* @brief : APS6408L-OBM-BA DTR Memory Mode 16-bit Read Write Test. 
           Write and Read 1MB as 16-bit Mode
* @param : write_data: the data write to PSRAM 
* @return: 0: PASS; 1: FAIL  
******************************************************************************/
uint16_t OSPI_DTR_Memory_16bit_Write_Read_Performance_Test(uint16_t write_data)
{    
    volatile uint16_t *pMem16;
    uint32_t i, ms_tick1, ms_tick2, ms_tick3, ms_tick4;   
 
    printfS("\r\n16bit_Write_Read_Performance_Test, 1M Bytes, Test Data: 0x%04x\r\n", write_data);    
...
* @return: 0: PASS; 1: FAIL  
******************************************************************************/
uint16_t OSPI_DTR_Memory_32bit_Write_Read_Performance_Test(uint32_t write_data)
{    
    volatile uint32_t *pMem32;
    uint32_t i, ms_tick1, ms_tick2, ms_tick3, ms_tick4;   
 
    printfS("\r\n32bit_Write_Read_Performance_Test, 1M Bytes, Test Data: 0x%08x\r\n", write_data);    
...
/******************************************************************************
* @brief : APS6408L_OBM_OSPI_PSRAM_Performance_Byte.    
* @param : None
* @return: None
******************************************************************************/
void APS6408L_OBM_OSPI_PSRAM_Performance_Byte(void)
{
    uint16_t result = 0, test_loop;    

    test_loop = 0;
    
    while(test_loop++ < 3)
    {
        result = OSPI_DTR_Memory_8bit_Write_Read_Performance_Test(0xAA);
        if (0 != result)
        {
            printfS("0xAA Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_8bit_Write_Read_Performance_Test(0x55);
        if (0 != result)
        {
            printfS("0x55 Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_8bit_Write_Read_Performance_Test(0xCC);
        if (0 != result)
        {
            printfS("0xCC Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_8bit_Write_Read_Performance_Test(0x33);    
        if (0 != result)
        {
            printfS("0x33 Test ERROR, Test End\n"); 
            while(1)
            {
...
/******************************************************************************
* @brief : APS6408L_OBM_OSPI_PSRAM_Performance_HalfWord.  
* @param : None
* @return: None
******************************************************************************/
void APS6408L_OBM_OSPI_PSRAM_Performance_HalfWord(void)
{
    uint16_t result = 0, test_loop;      
    ...
    while(test_loop++ < 3)  
    {
        result = OSPI_DTR_Memory_16bit_Write_Read_Performance_Test(0xAA55);
        if (0 != result)
        {
            printfS("0xAA55 Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_16bit_Write_Read_Performance_Test(0x55AA);
        if (0 != result)
        {
            printfS("0x55AA Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_16bit_Write_Read_Performance_Test(0xCC33);
        if (0 != result)
        {
            printfS("0xCC33 Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_16bit_Write_Read_Performance_Test(0x33CC);
        if (0 != result)
        {
            printfS("0x33CC Test ERROR, Test End\n"); 
            while(1)
            {
...
/******************************************************************************
* @brief : APS6408L_OBM_OSPI_PSRAM_Performance_Word.  
* @param : None
* @return: None
******************************************************************************/
void APS6408L_OBM_OSPI_PSRAM_Performance_Word(void)
{
    uint16_t result = 0, test_loop;  
    ...
    while(test_loop++ < 3)  
    {
        result = OSPI_DTR_Memory_32bit_Write_Read_Performance_Test(0xAA5555AA);
        if (0 != result)
        {
            printfS("0xAA5555AA Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_32bit_Write_Read_Performance_Test(0x55AAAA55);
        if (0 != result)
        {
            printfS("0x55AAAA55 Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_32bit_Write_Read_Performance_Test(0xCC3333CC);
        if (0 != result)
        {
            printfS("0xCC3333CC Test ERROR, Test End\n"); 
            while(1)
            {
        ...
        result = OSPI_DTR_Memory_32bit_Write_Read_Performance_Test(0x33CCCC33);
        if (0 != result)  
        {
            printfS("0x33CCCC33 Test ERROR, Test End\n");   
            while(1)
            {
...
{   
    ...
    APS6408L_OBM_ReadID();
    //寄存器测试
    APS6408L_OBM_ReadRegister_Test();
    //FIFO模式测试
    APS6408L_OBM_Fifo_Read_Write();     
    //Memory模式测试
    APS6408L_OBM_OSPI_DTR_Memory_Init();
    
    APS6408L_OBM_OSPI_PSRAM_Performance_Byte();   
    APS6408L_OBM_OSPI_PSRAM_Performance_HalfWord(); 
    APS6408L_OBM_OSPI_PSRAM_Performance_Word();  
