...
f_save = open('./all_log.txt','w')
def print2file(tempstr):
    print(tempstr)
    if 1:
        print(tempstr,file=f_save)
		...
		for line in lines:
			if key in line:
				...
				if num_eroor == '0':
					...
					print2file("KEIL编译错误，Error(s)："+num_eroor+" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n")
					result= False
		if have_output==0:
			print2file("错误，没有输出信息")
			result= False
	else:
		key='errors'
		for line in lines:
			if key in line:
				...
				if num_eroor == '0':
					...
					print2file("IAR编译错误，Error(s)："+num_eroor+" >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>\n")
					result= False
		if have_output==0:
			print2file("错误，没有输出信息")
			result= False
...
def StartBulid(workpath, List1, filetype, Path_build,IDE_Type):
	...
	num_current=0
	num_BuildOK=0
	print2file("编译器类型"+IDE_Type+" 一键编译开始,工程总数量： "+"  "+str(num_project))
	for A in List1:
		num_current+=1
		print2file("当前工程:("+IDE_Type+") "+str(num_current)+"/"+str(num_project))
		print2file("开始编译工程："+A)
	...
	dirListRemove = []
	getAllDir(path,dirListRemove,('Objects','Listings', 'Out_Files'))
...
def runbuild():
	...
	print2file(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  END!   >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>.")
	print2file(result_KEIL)
	print2file(result_IAR)
